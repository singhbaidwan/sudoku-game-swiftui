...
//  sudoku
//
//  Created by Dalveer singh on 15/12/22.
//
...
class BoardClass:ObservableObject{
    ...
    func chooseNextPuzzle(){
        ...
        while(prevPuzzle==currentPuzzle)
        {
    ...
    func makeNewBoard(){
        ...
        for (i,ch) in currentPuzzle[0].map(String.init).enumerated(){
            items.append(Cell(value: (Int(ch) ?? 0), index: i, rowCol: indexToRowCol(index: i), isPermanent: (Int(ch) ?? 0) > 0))
        }
    }
    ...
    func setSquare(index:Int,newVal:Int){
    ...
    func isValid(rowIndex:Int,colIndex:Int,k:Int)->Bool{
        for i in 0..<9{
            ...
            if(items[rowColToIndex(row: rowIndex, col: i)].cellValue==k ||
               items[rowColToIndex(row: i, col: rowIndex)].cellValue == k ||
               items[rowColToIndex(row: m, col: n)].cellValue == k
            ){
                return false
            }
    ...
    func revealOne(){
        for (i,ch) in currentPuzzle[1].map(String.init).enumerated(){
            if(items[i].cellValue == 0)
            {
                items[i].cellValue = Int(ch) ?? 0
                items[i].isHint = true
                break
            }
        }
    ...
    func checkBoard(){
        for (i,ch) in currentPuzzle[1].map(String.init).enumerated(){
            if(items[i].cellValue > 0 && items[i].cellValue != Int(ch))
            {
                items[i].isWrong = true
            }
            else
            {
                items[i].isWrong = false
                items[i].cellValue = Int(ch) ?? 0
            }
        }
    }
